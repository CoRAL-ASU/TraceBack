...
def _load_dotenv_from_root():
	...
	try:
		...
		# Fail silent; env vars can still be provided by the shell
		return
...
class Call_OpenAI:
	...
	def call(self, prompt):
		try:
			...
			model=self.model,
			messages=[
				{"role": "developer", "content": "You are a helpful, instruction-following assistant."},
				{
					"role": "user",
					"content": prompt
				}
			...
			# Propagate the error so callers don't silently get None
			raise
...
class Call_Gemini:
	def __init__(self, model_name='gemini-2.0-flash-exp'):
		...
		# GEMINI_API_KEYS: comma-separated list, or GEMINI_API_KEY: single key
	...
	def call(self, prompt):
		...
		# print('Gemini Model Input Prompt Token Count: ', model.count_tokens(prompt))
	...
	def call(self, prompt):
		...
		try:
			response = self.client.chat.completions.create(
				...
				messages=[
					{"role": "system", "content": "You are an expert at instruction-following.Just output the final answer, nothing else."},
...
class Call_Novita:
	...
	def _throttle(self) -> None:
		...
		if wait > 0:
			time.sleep(wait)
		# Next request is allowed after min-interval from *this* request start.
	...
	def call(self, prompt: str) -> str:
		...
		if "qwen3" in str(self.model).lower():
			# Best-effort: discourage thinking mode. If the endpoint still returns <think> blocks,
			# we strip them below.
		...
		for attempt in range(max(1, self.max_retries) + 1):
			...
			try:
				...
				# Exponential backoff + jitter, but always wait at least the RPM interval.
...
class Call_HF:
	...
	def __init__(
		...
		*,
		...
		# Ensure we have a pad token to suppress generation warnings.
		try:
